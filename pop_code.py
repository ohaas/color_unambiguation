...
#                        ax.add_patch(fap((x,y),((x+(self.square_size/4)*x1/ny.sqrt(x1**2+y1**2)),((y+(self.square_size/4)*y1/ny.sqrt(x1**2+y1**2)))), arrowstyle='->',linewidth=0.5,mutation_scale=10))
                        ...
                        pp.xticks(ny.arange(0,30,5))
                        pp.yticks(ny.arange(0,30,5))
                        pp.xlabel('Pixel')
                        pp.ylabel('Pixel')
                        if i>0:
    ...
    def plot_pop(self, population_code, time_frames, i):
        ...
        for x in ny.arange(0.0,self.main_size):
            for y in ny.arange(0.0,self.main_size):
                if not ny.any(population_code[x,y,:])==0:
                    ...
                    y2=ny.arange(0,ny.max(ny.abs(population_code[x,y,:]))+0.01,0.01) # vector length is maximum of all neurons per pixel! not value for angle direction... change to mean or so???
                    x2=angle+(0.0*y2)
        ...
        y2=ny.arange(0,ny.max(ny.abs(population_code[:,:,:]))*1.01,0.01)
        #pp.axhline(xmax=1.0, linewidth=0.7, color='w', linestyle='--')
        pp.plot(0*(math.pi/180)+(0.0*y2),y2,'w', linestyle='--')
        pp.plot(45*(math.pi/180)+(0.0*y2),y2,'w', linestyle='--')
        pp.plot(90*(math.pi/180)+(0.0*y2),y2,'w', linestyle='--')
        pp.plot(135*(math.pi/180)+(0.0*y2),y2,'w', linestyle='--')
        pp.plot(180*(math.pi/180)+(0.0*y2),y2,'w', linestyle='--')
        pp.plot(225*(math.pi/180)+(0.0*y2),y2,'w', linestyle='--')
        pp.plot(270*(math.pi/180)+(0.0*y2),y2,'w', linestyle='--')
        pp.plot(315*(math.pi/180)+(0.0*y2),y2,'w', linestyle='--')
    ...
    def heat_map(self, population_code, i, time_frames, A):
        ...
        if i>0:
            ...
            frame2 = fig.add_subplot(math.floor(time_frames/3.0)+1,3,i)
...
if __name__ == '__main__':
    ...
    p = Population(30, 4, 30, 16, angles, 135, 90)
